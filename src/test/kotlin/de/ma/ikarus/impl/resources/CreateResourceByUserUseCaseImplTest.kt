...
class CreateResourceByUserUseCaseImplTest {
    ...
    fun `creates an resource with with no validation errors`() {
        ...
        every { userGateway.addResourceToUser(user, resourceShow) } returns resourceShow
        ...
        verify(exactly = 1) { validator.validate(resourceCreate.toDTO()) }
        verify(exactly = 1) { resourceGateway.createResource(any()) }
        verify(exactly = 1) { userGateway.addResourceToUser(user, resourceShow) }
